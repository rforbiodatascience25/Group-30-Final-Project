...
First we calculate means from the three datapoints of both the 24h & 150h measurements.

```{r}
# Compute means per subject and duration
means <- saa_data_tidy %>%
...
# Join means back to long dataset
...
saa_rule_237 <- function(x) {
  ...
  if (n_ge_3000 >= 2 & n_ge_45000 <= 1) return("Positive")
